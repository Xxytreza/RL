...
class SarsaAgent:
    ...
    def get_value(self, state: State) -> float:
        ...
        value = float('-inf')
        # BEGIN SOLUTION
        for l_action in self.legal_actions:
            value = max(value, self.get_qvalue(state, l_action))
        # END SOLUTION
        return value
    ...
    def update(
        ...
        q_old_next_state = self.get_qvalue(next_state, next_action)
        q_old = self.get_qvalue(state, action)

        td_target = reward + self.gamma * q_old_next_state
